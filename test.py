...
def view_reports():
    if 'id' in session and session['role'] == 'admin':
        ...
        # Fetch unique months and years for dropdown
        cursor.execute("SELECT DISTINCT MONTH(date) AS month, YEAR(date) AS year FROM report ORDER BY year DESC, month ASC")
        ...
        # Render the template with reports and dropdown data
        return render_template(
            'view_reports.html',
            reports=reports,
            dates=dates,
